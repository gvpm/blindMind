...
//-----------------------------------------------------------------------------------------------
           ...
           //trying to bring levels to the new model
            //problem in those values
           minBlind=0;
           maxBlind=20;
           reactionTresh=10;

            drawBlindImageNew(minBlind,maxBlind,reactionTresh);
            ...
            if(lvl==0){
               ...
               ofSetColor(255,0,0);
                ...
                ///LVL 6
                    ...
                    maxBlind=45;
                    reactionTresh=90;
                    //reactionTresh = ofMap(mouseX,0,ofGetWidth(),0,255,true);
                cout << reactionTresh;
                    drawBlindImageNew(minBlind,maxBlind,reactionTresh);
...
//will get the color balanced according to the distances to the real sensors
    int ofApp::getBalancedColor (int x, int y){
        ...
        */
         ...
         //printf("%f  ",balanced);
         //printf("  ");
         balanced = ofMap(balanced, 0, 255,0,255,true);
         //printf("%f  ",balanced);
         //printf("  ");
    ...
    //-----------------------------------------------------------------------
        ...
        void ofApp::drawBlindImageNew(int minBlind,int maxBlind,int reactionTresh){
           ...
           for(int i = 0; i<ofGetWidth();i+=15){
               ...
               for(int j = 0; j<ofGetHeight();j+=15){
                   ...
                   if(getBalancedColor(i,j)>(255-reactionTresh)){
                       grayColor = ofMap(getBalancedColor(i,j),(255-reactionTresh),255,minBlind,maxBlind);
                       }else {
                      grayColor=minBlind;
                   }

                   ofSetColor(grayColor);
                   //ofSetColor(getBalancedColor(i,j));
